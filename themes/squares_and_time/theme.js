...
function renderFrame() {
    ...
    for (let i = 1; i <= cellsPerLine; i++) {
      for ( let j = 1; j <= cellsPerLine; j++) {
        context.fillStyle = "hsl(" + colours[i * cellsPerLine + j] + ", 100%, 50%)";
        colours[i * cellsPerLine + j] += (10);
        if (colours[i * cellsPerLine + j] > 360) colours[i * cellsPerLine + j] = colours[i * cellsPerLine + j] % 360;
        context.fillRect(cellSize * (i-1), cellSize * (j-1), cellSize, cellSize);
      }
    }
    context.filter = "none";
    ...
    return canvas.toBuffer("image/jpeg", 70).toString("base64");
}
