...
class Sensors {
  ...
  listHardware() {
    if (this.libreRunning) {
      ...
      return [
        {
          path: "/no-sensors/0",
          name: "Check if LibreHardwareMonitor is running.",
        },
      ];
    }
  }
  ...
  provideExistingSensor() {
    const sensors = this.query(
      'Select Identifier From Sensor Where Identifier Like "/%cpu%/%/temperature/%"'
    );
