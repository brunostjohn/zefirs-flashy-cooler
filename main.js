...
let renderPath = path.join(__dirname, "/stock_themes/clock/theme.js");

const worker = new Worker("./libraries/renderer.js", { workerData: {renderPath: renderPath, fps: fps} });
...
const createTray = () => {
    ...
    const contextMenuInactive = Menu.buildFromTemplate([
        ...
        { label: 'Start Rendering', click() {tray.setContextMenu(contextMenuActive); worker.postMessage("start"); } },
        { label: 'Quit CapellixLCD', click() { worker.postMessage("exit"); app.exit(0); } }
    ])

    const contextMenuActive = Menu.buildFromTemplate([
        { label: 'Open CapellixLCD', click() { createWindow(); }},
        { label: 'Stop Rendering', click() {tray.setContextMenu(contextMenuInactive); worker.postMessage("stop");} },
        { label: 'Quit CapellixLCD', click() { worker.postMessage("exit"); app.exit(0); } }
    ])
